...
{
	printf("[%2d ", test_num);
	if (!test_lexer_00_basic(line, test_result))
		printf(GRN"ok"RESET"]");
	else
		printf(RED"ng"RESET"]");
}
...
void	execute_test_00(void)
{
	...
	execute_test(25, line_25, test_result_25);

	char	*line_26 = "echo m\"test\\\"\"";
	char	*test_result_26[] = {"echo", "m\"test\\\"\"", NULL};
	execute_test(26, line_26, test_result_26);
}
