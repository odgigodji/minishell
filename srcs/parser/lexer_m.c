...
int			get_token3(char *line, char **token, t_common *common)
{
	...
	while (line[count_line])
	{
		...
		if (line[count_line] == '\\' && !flag_back_slash && !flag_brace_single && (!flag_brace_double || ft_strchr("$\\\"\'", line[count_line + 1])))
		{
			flag_back_slash = 1;
			count_line++;
			continue ;
		...
		if ((line[count_line] == '~' || (line[count_line] == '$')) && !flag_brace_single && !flag_back_slash) // && (ft_isalnum(line[count_line + 1]) || ft_strchr("_$?", line[count_line + 1]))
		{
